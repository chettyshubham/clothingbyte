	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().exceptionHandling().authenticationEntryPoint((request, response, ex) -> {
			response.sendError(HttpServletResponse.SC_UNAUTHORIZED, ex.getMessage());
				...
				.antMatchers("/api/orders/*", "/api/sellers/signin", "/api/sellers/signup", "/api/admin/signin",
						"/api/customers/signin", "/api/customers/signup", "/api/admin/save",
						"/api/products/allproducts", "/api/products/image/*", "/api/products/cats")
